...
interface Stage    { name: string; description?: string }
interface Progress { stage: string; completed_at: string | null; score: number }
interface Config   { frameworkName: string; stages: Stage[] }

export default function DashboardPage() {
  const { user } = useUser();
  const [config,   setConfig]   = useState<Config>({ frameworkName: "My Framework", stages: [] });
  ...
  async function markComplete(stageName: string) {
    ...
    if (res?.ok) {
      ...
      setProgress(prev => [...prev.filter(p => p.stage !== stageName), data.progress]);
    }
    setMarking(null);
  }

  const { frameworkName: FW, stages: STAGES } = config;
  const completedNames  = new Set(progress.filter(p => p.completed_at).map(p => p.stage));
  const completedCount  = completedNames.size;
  const completionPct   = STAGES.length > 0 ? Math.round((completedCount / STAGES.length) * 100) : 0;
    ...
    <div className="min-h-screen bg-[#0A0A0F] text-[#F0F0F0]">
      <header className="border-b border-[#1E1E2E] px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center justify-between">
          <div>
            <p className="text-[9px] font-mono tracking-[0.25em] text-[#C9A84C]/60 uppercase mb-0.5">{FW}</p>
            <h1 className="text-lg font-black">Welcome back{user?.firstName ? `, ${user.firstName}` : ""}</h1>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        {STAGES.length === 0 ? (
          <div className="text-center py-20 text-[#6B7280]">Loading your framework…</div>
        ) : (
              ...
              return (
                <div
                  ...
                  className={`p-5 rounded-2xl border transition-all ${
                    isComplete ? "border-[#00E5A0]/30 bg-[#00E5A0]/5"
                ...
                >
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {isComplete && <span className="text-[9px] font-bold text-[#00E5A0] bg-[#00E5A0]/10 border border-[#00E5A0]/30 px-2 py-0.5 rounded-full">DONE ✓</span>}
                    {isActive   && <span className="text-[9px] font-bold text-[#7C3AED] bg-[#7C3AED]/10 border border-[#7C3AED]/30 px-2 py-0.5 rounded-full">ACTIVE</span>}
                    {isLocked   && <span className="text-[9px] font-bold text-[#6B7280] bg-[#1E1E2E] px-2 py-0.5 rounded-full">LOCKED</span>}
                  </div>
                  <h3 className="font-bold text-[#F0F0F0] mb-1">{stage.name}</h3>
                  {stage.description && <p className="text-xs text-[#6B7280] mb-3">{stage.description}</p>}
                  {isActive && (
                    <button
